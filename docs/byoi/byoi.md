...
1. [Sign your image by using Docker Content Trust](#sign_image_dct)
2. [Adding the registry](#register-add)
3. [Generating the signing keys](#signing_key_gen)
4. [Preparing the configuration](#config_byoi_prep)
3. [Deploy your image](#deploying-your-own-image)
...
4. Push your images to the DockerHub by running the following command.
   ```
...
??? example "Virtual server template file"
...
2. Create the configuration yaml file $HOME/hpvs/config/demo_byoi.yml for the instance by referring to the example file $HOME/hpvs/config/vs_regfiledeployexample.yml.

   ??? example "vs_regfiledeployexample.yml file"
